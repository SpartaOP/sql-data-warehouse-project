/*
	Cleaning & inserting data into silver schema tables from bronze schema tables.
...
BEGIN
	...
	BEGIN TRY
		...
		SELECT 
			cst_id,
			cst_key,
			TRIM(cst_firstname) AS cst_firstname, -- Eliminating extra spaces
			TRIM(cst_lastname) AS cst_lastname, -- Eliminating extra spaces
		...
		)t WHERE flag_last = 1; -- Eliminating dpulicate primary key (customer id in this case)
		...
		SELECT
			prd_id,
			REPLACE(SUBSTRING(prd_key, 1, 5), '-', '_') AS cat_id, -- Extracting category id from product key and replacing '-' with '_' for joining purposes
			SUBSTRING(prd_key, 7, LEN(prd_key)) AS prd_key, -- Extracting remaining substring from product key
			TRIM(prd_nm) AS prd_nm, -- Eliminate extra spaces
			...
			CASE UPPER(TRIM(prd_line))
				...
				ELSE 'na' -- Data Standardization
			END AS prd_line,
			CAST(prd_start_dt AS DATE) AS prd_start_dt, -- Typecasting to DATE as no TIME provided
			CAST(LEAD(prd_start_dt) OVER (PARTITION BY prd_key ORDER BY prd_start_dt)-1 AS DATE) AS prd_end_dt -- Calculating end date as one day before the next start date for the same product key
		...
		SELECT
			...
			END AS sls_order_dt, -- Typecasting to DATE and handling invalid date formats
			CASE WHEN sls_order_dt = 0 OR LEN(sls_ship_dt) !=8 THEN NULL
				 ELSE CAST(CAST(sls_ship_dt AS VARCHAR) AS DATE)
			END AS sls_ship_dt, -- Typecasting to DATE and handling invalid date formats
			CASE WHEN sls_due_dt = 0 OR LEN(sls_due_dt) !=8 THEN NULL
				 ELSE CAST(CAST(sls_due_dt AS VARCHAR) AS DATE)
			END AS sls_due_dt, -- Typecasting to DATE and handling invalid date formats
		...
		SELECT 
			...
			CASE WHEN TRIM(cntry) = 'DE' THEN 'Germany'
				 ...
				 ELSE TRIM(cntry)
			END AS cntry
		FROM bronze.erp_loc_a101 -- Data Standardization
